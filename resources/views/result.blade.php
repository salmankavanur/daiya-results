...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .glass-panel, .data-card { background: none !important; border: none !important; box-shadow: none !important; filter: none !important; backdrop-filter: none !important; }
    ...
    <div class="relative z-10 flex flex-col min-h-screen">
        ...
        <main class="flex-grow flex justify-center items-start px-4 py-8">
            <div class="w-full max-w-5xl">
                <div class="glass-panel rounded-[2.5rem] p-8 sm:p-12 relative overflow-hidden print-border">
                    ...
                    <!-- Report Header -->
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-white/10 pb-8 mb-10 relative z-10">
                        <div>
                            <div class="inline-block px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 text-xs font-bold tracking-widest uppercase mb-4">
                                Official Statement of Marks
                            </div>
                            <h1 class="text-4xl sm:text-5xl font-extrabold text-white tracking-tight mb-2">Academic Result</h1>
                            <p class="text-xl text-gray-400 font-medium">{{ $result->batch }}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 relative z-10">
                        <div class="data-card p-6 rounded-3xl flex items-center gap-5">
                            <div class="w-14 h-14 rounded-full bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20 text-indigo-400">
                            ...
                            <div>
                                <p class="text-sm text-gray-400 font-medium mb-1 uppercase tracking-wider">Candidate Name</p>
                                <p class="text-2xl font-bold text-white">{{ $result->name }}</p>
                            </div>
                        </div>
                        <div class="data-card p-6 rounded-3xl flex items-center gap-5">
                            <div class="w-14 h-14 rounded-full bg-purple-500/10 flex items-center justify-center border border-purple-500/20 text-purple-400">
                                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2"></path></svg>
                            </div>
                            <div>
                                <p class="text-sm text-gray-400 font-medium mb-1 uppercase tracking-wider">Registration Number</p>
                                <p class="text-2xl font-bold text-white uppercase tracking-wider">{{ $result->reg_no }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-12 relative z-10">
                        ...
                        <div class="data-card rounded-3xl overflow-hidden">
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="border-b border-white/5 bg-white/5">
                                        <th class="py-5 px-8 font-semibold text-gray-300 uppercase tracking-wider text-sm">Subject</th>
                                        <th class="py-5 px-6 font-semibold text-gray-300 uppercase tracking-wider text-sm text-center">Term End (TE)</th>
                                        <th class="py-5 px-6 font-semibold text-gray-300 uppercase tracking-wider text-sm text-center">Cont. Eval (CE)</th>
                                        <th class="py-5 px-8 font-semibold text-indigo-300 uppercase tracking-wider text-sm text-center bg-indigo-500/10">Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($result->marks_data as $subject => $marks)
                                        ...
                                        <tr class="border-b border-white/5 table-row-hover transition-colors">
                                            <td class="py-5 px-8 font-medium text-gray-200">{{ $subject }}</td>
                                            <td class="py-5 px-6 text-center text-gray-400 font-mono">{{ $marks['TE'] ?? '-' }}</td>
                                            <td class="py-5 px-6 text-center text-gray-400 font-mono">{{ $marks['CE'] ?? '-' }}</td>
                                            <td class="py-5 px-8 text-center font-bold text-white font-mono bg-indigo-500/5">{{ $subTotal > 0 ? $subTotal : '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-5 relative z-10">
                        ...
                        <div class="p-6 rounded-3xl bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 relative overflow-hidden group">
                            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                <svg class="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z"></path></svg>
                            </div>
                            <p class="text-slate-400 text-sm font-medium mb-2 uppercase tracking-widest relative z-10">Total Marks</p>
                            <p class="text-3xl font-black text-white relative z-10">{{ $result->total_obt_marks ?? '-' }} <span class="text-lg font-medium text-slate-500">/ {{ $result->total_marks ?? '-' }}</span></p>
                        </div>
                        @endif

                        @if($result->daiya_rank)
                        <div class="p-6 rounded-3xl bg-gradient-to-br from-indigo-900 to-indigo-950 border border-indigo-800 relative overflow-hidden group">
                            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                            ...
                            <p class="text-indigo-300 text-sm font-medium mb-2 uppercase tracking-widest relative z-10">Daiya Rank</p>
                            <p class="text-3xl font-black text-white relative z-10">#{{ $result->daiya_rank }}</p>
                        </div>
                        @endif

                        @if($result->college_rank)
                        <div class="p-6 rounded-3xl bg-gradient-to-br from-purple-900 to-purple-950 border border-purple-800 relative overflow-hidden group">
                            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                            ...
                            <p class="text-purple-300 text-sm font-medium mb-2 uppercase tracking-widest relative z-10">College Rank</p>
                            <p class="text-3xl font-black text-white relative z-10">#{{ $result->college_rank }}</p>
                        </div>
                        @endif
                        ...
                        @php
                            ...
                            $statusGradient = $isPass ? 'from-emerald-900 to-emerald-950 border-emerald-800 text-emerald-300' : 'from-rose-900 to-rose-950 border-rose-800 text-rose-300';
                            $iconColor = $isPass ? 'text-emerald-300' : 'text-rose-300';
                        @endphp
                        <div class="p-6 rounded-3xl bg-gradient-to-br {{ $statusGradient }} border relative overflow-hidden group">
                            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                @if($isPass)
                            ...
                            <p class="{{ $isPass ? 'text-emerald-300/80' : 'text-rose-300/80' }} text-sm font-medium mb-2 uppercase tracking-widest relative z-10">Final Status</p>
                            <p class="text-3xl font-black text-white uppercase relative z-10 tracking-wide">{{ $result->status }}</p>
                        </div>
                        @endif
                    </div>
